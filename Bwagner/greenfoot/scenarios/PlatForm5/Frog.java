...
public class Frog extends Actor
{
    ...
    private boolean releaseShark = false;  // this flag prevents more than one shark from being released
    ...
    public void endGameTest()
    {
        ...
        if(isTouching(Ground.class) && releaseShark == false)
        {
           dx = 0;
           dy = 0;
           gravity = 0;
           
           // release shark from right side of the world
           Shark shark = new Shark();  // create shark
           getWorld().addObject(shark, getWorld().getWidth() + shark.getImage().getWidth()/2,   // add shark off screen to the right
                                getWorld().getHeight() - shark.getImage().getHeight()/2 + 20);
           releaseShark = true;   // this flag prevents more than one shark from being released 
        }
    }
    
    /**
     * Test to see if fish has collide with frog.
     * If frog collides with the fish's bottom the frog falls at a speed of 2 units.
     * If fish collides with the frog from the right frog is bumped 10 units to the left.
     * If fish collides with the frog from the left frog is bumped 10 units to the right.
     */
    public void fishTest()
    {
        Actor actor = null;  // used to store fish
       
        // get Fish right above frog if it exists
       actor = getOneObjectAtOffset(0, -1 * (getImage().getHeight()/2 + 5), Fish.class);
       if(actor != null)  // was there a fish above frog?
       {
           dy = 2;  // make frog fall
       }
       
       // get Fish to the right of the frog if it exists
       actor = getOneObjectAtOffset((getImage().getWidth()/2 + 5), 0, Fish.class);
       if(actor != null)  // was there a fish to right of frog? 
       {
           setLocation(getX() - 10, getY());  // bump frog 10 units to left
       }
       
       // get Fish to the left of the frog if it exists
       actor = getOneObjectAtOffset(-1 * (getImage().getWidth()/2 + 5), 0, Fish.class);
       if(actor != null)  // was there a fish to left of frog?
       {
           setLocation(getX() + 10, getY());  // bump frog 10 units to right
       }
